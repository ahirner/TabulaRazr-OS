        ...
        if row_qualifies(row):
            ...
            if underqualified > grace_rows:
                if consecutive >= min_consecutive:
                    #txt = " ".join([r['value'] for r in row_features[last_qualified]])
                    #print ("last qualified %i, row counters %i/%i, underqualified before %i, consecutive %i: %s" \
                    #       % (last_qualified, i, j, underqualified, consecutive, txt))
...
#t = return_tables("./static/ug/txt/EP849915-EP657701-EP1059361.pdf.txt")
